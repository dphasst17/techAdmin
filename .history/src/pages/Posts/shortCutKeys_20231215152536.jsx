    ...
    return <div className="shortKey-container md:w-[90%] h-auto flex flex-wrap justify-around bg-slate-500 rounded-lg px-2">
        {shortKey.map(e => <div className={`${e.name}  w-[23%] h-auto flex flex-col`} key={e.name}>
            {e.detail.map(d => <span className="w-full flex items-center text-white font-semibold my-2" key={d.result}>
                <div className="w-[40px] h-[20px] flex justify-center items-center text-slate-900 bg-slate-400 rounded-lg mx-2 cursor-pointer hover:bg-slate-700 hover:text-slate-300 transition-all">{d.first}</div> + 
