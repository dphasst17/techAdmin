            ...
            <div className="formView w-full h-[90%] flex flex-col justify-start overflow-y-auto">
                ...
                <form className="w-full h-auto overflow-y-auto flex flex-wrap justify-center">
                    ...
                    <div className="formProduct w-[98%] h-auto min-h-[50px] flex flex-wrap justify-center items-start px-5">
                        {inputs.map((input) => (
                            <div className="w-4/5 h-auto lg:h-[40px] flex flex-wrap justify-around my-4" key={input}>
                                <input
                                    className="w-2/4 lg:w-1/4 h-[40px] lg:h-full rounded-lg outline-none px-2"
                                    name={`name-${input}`}
                                    {...register(`input${input}`, { required: false })}
                                    placeholder="Name"

                                />
                                <select className="w-2/4 lg:w-1/4 h-[40px] lg:h-full rounded-lg" name="select" {...register(`option${input}`, { required: false })}>
                                ...
                                />
                                <button onClick={(e) => {e.preventDefault();delCol(input)}} className="w-2/4 lg:w-[15%] h-[40px] lg:h-full bg-red-700 text-white font-bold rounded-lg">Delete column</button>
                            </div>
                        ))}
