                    ...
                    <div className="formProduct w-[98%] h-auto min-h-[50px] flex flex-wrap justify-center items-start px-5">
                        {inputs.map((input) => (
                            <div className="w-full md:w-4/5 h-auto lg:h-[40px] flex flex-wrap justify-around my-4" key={input}>
                                <input
                                    className="w-[45%] lg:w-1/4 h-[40px] lg:h-full my-1 rounded-lg outline-none px-2"
                                    name={`name-${input}`}
                                    {...register(`input${input}`, { required: false })}
                                    placeholder="Name"

                                />
                                <select className="w-[45%] lg:w-1/4 h-[40px] lg:h-full my-1 rounded-lg" name="select" {...register(`option${input}`, { required: false })}>
                                ...
                                />
                                <button onClick={(e) => {e.preventDefault();delCol(input)}} className="w-[45%] lg:w-[15%] h-[40px] lg:h-full my-1 bg-red-700 text-white font-bold rounded-lg">Delete column</button>
                            </div>
                        ))}
