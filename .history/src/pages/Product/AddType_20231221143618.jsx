    ...
    return <div className="formProduct w-screen h-screen flex items-center justify-center fixed z-50 top-0">
        ...
        <div className="formDetail md:w-[60vw] h-[70vh] lg:h-[50vh] bg-slate-500 z-40 flex flex-col justify-start items-center pt-12 md:pt-0 pb-5 rounded-xl">
            ...
            <div className="formView w-full h-[90%] flex flex-col justify-start overflow-y-auto">
                <button className="w-[100px] h-[30px] rounded-lg bg-green-500 mx-2 font-bold text-gray-600 hover:text-gray-900 hover:bg-green-400 transition-all" type="button" onClick={handleClick}>Add column</button>
                <form className="w-full h-auto overflow-y-auto flex flex-wrap justify-center">
                    <input type="text" {...register('tbName', { required: true })} className={`w-3/4 h-[40px] my-4 ${err.tbName ? 'border-solid border-red-500 border-[1px]' :'border-green-500 border-solid border-[1px]'} rounded-lg outline-none px-2`} placeholder="Name type..." />
                    <div className="formProduct w-[98%] h-auto min-h-[50px] flex flex-wrap justify-center items-start px-5">
                        {inputs.map((input) => {
                            ...
                            return(
                            <div className="w-full md:w-[95%] h-auto lg:h-[40px] flex flex-wrap justify-around my-4" key={input}>
                                <input
                                    className={`w-[45%] lg:w-1/5 h-[40px] lg:h-full ${err[`name-${input}`] ? 'border-solid border-red-500 border-[1px]' :'border-transparent'} my-2 rounded-lg outline-none px-2`}
                                    name={`name-${input}`}
                                    {...register(`name-${input}`, { required: true })}
                                    placeholder="Name"

                                />
                                <select className="w-[45%] lg:w-1/5 h-[40px] lg:h-full my-2 rounded-lg" name="select" {...register(`option${input}`, { required: false })}>
                                ...
                                <input 
                                    className={`w-[45%] lg:w-1/5 h-[40px] ${err[`input-demo${input}`] ? 'border-solid border-red-500 border-[1px]' :'border-transparent'} lg:h-full my-2 rounded-lg outline-none px-2`}
                                    name={`limit-${input}`}
                                    {...register(`input-demo${input}`, { required: watchOption === 'longtext' || watchOption === 'date' ? false : true })}
