    ...
    return <>
        ...
        <div className="diary w-full h-auto flex flex-wrap justify-evenly">
            ...
            {dataDiary !== null && dataDiary.slice(numSlice - 14, numSlice).map(e => <div className="w-full xl:w-4/5 h-auto min-h-[50px] md:h-[50px] flex flex-wrap justify-center items-center bg-slate-300 rounded-lg my-2" key={e.id}>
                ...
                <span className="w-1/4 md:w-[15%] h-[50px] md:h-full flex items-center justify-center text-[18px] text-slate-700 font-semibold">{e.idpersonIOX}</span>
                <span className="w-1/4 md:w-[15%] h-[50px] md:h-full flex items-center text-[18px] text-slate-700 font-semibold overflow-hidden whitespace-nowrap text-ellipsis"><span className="overflow-hidden whitespace-nowrap text-ellipsis">{e.dateIOX}</span></span>
                <span className="w-1/4 md:w-[15%] h-[50px] md:h-full flex items-center justify-center text-[18px] text-slate-700 font-semibold">{e.countProduct}</span>
                <span className="w-1/4 md:w-[15%] h-[50px] md:h-full flex items-center justify-center text-[18px] text-slate-700 font-semibold">{e.statusWare}</span>
            </div>)}
        </div>
