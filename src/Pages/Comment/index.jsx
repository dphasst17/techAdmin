...
const Comment = () => {
    ...
    const { dataResult, err:errResult } = useGetData('comment','commentGetAll');
    useEffect(() => {
        dataResult && setDataComment(dataResult.lastResult)
        console.log(dataResult)
    },[dataResult])
    useEffect(() => {errResult && console.log(errResult)},[errResult])
    return <div className="comment-container w-screen sm:w-full h-auto pt-2 pr-0 sm:pr-8 z-10">
        ...
        <div className="c-items w-full h-auto flex flex-wrap justify-center md:justify-around items-center">
            {dataComment && dataComment.map(e => <div className="w-4/5 md:w-[32%] min-w-[150px] h-[250px] flex justify-around bg-slate-500 rounded-[10px] my-2" key={e.idProduct}>
